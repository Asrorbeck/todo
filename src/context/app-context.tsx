...
interface AppContextType {
  ...
  projects: Project[];
  addProject: (
    project: Omit<Project, "id" | "createdAt" | "lastActivity">
  ) => Promise<Project>;
  updateProject: (id: number, updates: Partial<Project>) => Promise<void>;
  deleteProject: (id: number) => Promise<void>;
  tasks: Task[];
  addTask: (
    task: Omit<Task, "id" | "createdAt" | "activityLog">
  ) => Promise<Task>;
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const updateProject = async (id: number, updates: Partial<Project>) => {
    ...
    setProjects(
      projects.map((p) =>
        p.id === id
          ? { ...p, ...updates, lastActivity: updateData.last_activity }
          : p
      )
    );
  };
  ...
  const updateTask = async (id: number, updates: Partial<Task>) => {
    ...
    if (updates.dueDate && updates.dueDate !== task.dueDate) {
      activityLog.push({
        action: `due date changed to ${new Date(
          updates.dueDate
        ).toLocaleDateString()}`,
        timestamp: now,
      });
